...
auto PLUGIN_API Controller::initialize(FUnknown* const context) -> tresult {
  ...
  for (const auto& [group_id, group] : common::kSchema) {
    for (const auto& [param_id, param] : group) {
      ...
      if (const auto* const num_param =
        ...
        parameters.addParameter(new LinearParameter(
            ...
            VST3::StringConvert::convert(reinterpret_cast<const char*>(
                                             num_param->GetShortName().c_str()))
                .c_str()));
...
auto PLUGIN_API Controller::setComponentState(IBStream* const state)
  ...
  for (const auto& [group_id, group] : common::kSchema) {
    for (const auto& [param_id, param] : group) {
      const auto vst_param_id = kParamsPerGroup * group_id + param_id;
      const auto& value = core_.parameter_state_.GetValue(group_id, param_id);
      if (const auto* const num_param =
              std::get_if<common::NumberParameter>(&param)) {
...
auto PLUGIN_API Controller::setParamNormalized(
  ...
  if (const auto* const num_param =
          std::get_if<common::NumberParameter>(&param)) {
    core_.parameter_state_.SetValue(group_id, param_id,
...
auto Controller::SetStringParameter(const ParamID vst_param_id,
  ...
  core_.parameter_state_.SetValue(group_id, param_id, value);
